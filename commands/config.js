...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    let Config = new MessageEmbed()
      ...
      .addField("Moody Role", GuildDB.DJ ? `<@&${GuildDB.DJ}>` : "Not Set", true)
      .setDescription(`
...
`);
    ...
    if (em._emoji.name === "1️⃣") {
      ...
      await client.sendTime(
        message.channel,
        "Please mention the role you want `Moody's` to have."
      );
      ...
      if (!role.mentions.roles.first())
        return client.sendTime(
          message.channel,
          "Please mention the role that you want for Moody only."
        );
      ...
      client.sendTime(
        message.channel,
        "Successfully saved Moody role as <@&" + role.id + ">"
      );
    }
  },
  ...
  SlashCommand: {
    options: [
      ...
      {
        name: "dj",
        description: "Check the Moody role",
        type: 1,
        required: false,
        options: [
          {
            name: "role",
            description: "Set the Moody role",
            type: 8,
            required: false,
          },
        ],
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(
            interaction,
            `Successfully changed the Moody role of this server to ${role.name}`
          );
        } else {
          /**
